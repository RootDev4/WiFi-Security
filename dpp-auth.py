...
def topology():
    ...
    onboard_device(sta1,cli_cmd,sta2,"wpa_cli -p /var/run/wpa_supplicant2","00:00:00:00:00:82", psk,codecs.encode(ssid),"{}/DevID50/CredentialChain/ca-chain.cert.pem".format(cwd))

    time.sleep(3)

    f = open("sta2_0.staconf","r+") # creates file if not exists
    lines = f.read()
    print("******************************\n")
